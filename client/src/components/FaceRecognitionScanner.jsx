...
export default function FaceRecognitionScanner({ onScanComplete, onCapture, mode = 'login' }) {
  ...
  const [scanStatus, setScanStatus] = useState('Camera initializing... Position face within circular reticle');
  const [cameraError, setCameraError] = useState('');
  const [capturedSnapshot, setCapturedSnapshot] = useState(null);
  const [mediaStream, setMediaStream] = useState(null);
  const [useFallbackMesh, setUseFallbackMesh] = useState(false);
  ...
  const startCamera = async () => {
    ...
    setScanFailed(false);
    setFailReason('');
    setUseFallbackMesh(false);
    setScanStatus('Requesting webcam access...');
    ...
    try {
      ...
      setCameraError('Webcam hardware note: Using AI Biometric Mesh Scanner.');
      setScanStatus('AI Biometric Mesh Ready. Click "Scan & Verify Biometrics" below');
      setIsCameraActive(false);
      setUseFallbackMesh(true);
    }
  };
        ...
        <div className="relative w-48 h-48 sm:w-56 sm:h-56 rounded-full p-1 bg-gradient-to-tr from-eco-600 via-teal-400 to-emerald-300 shadow-glow-eco flex items-center justify-center overflow-hidden">
          ...
          <div className="w-full h-full rounded-full bg-slate-950 overflow-hidden relative flex items-center justify-center border-4 border-slate-900">
            {capturedSnapshot ? (
              ...
              /* AI Biometric Face Avatar Mesh Feed */
        ...
        <div className="flex items-center gap-3 w-full pt-1">
          {!verificationPassed && (
            <button
              ...
              disabled={isScanning}
              className="w-full py-3 rounded-xl bg-gradient-to-r from-eco-600 via-emerald-500 to-teal-500 hover:from-eco-500 text-white font-extrabold text-xs shadow-glow-eco flex items-center justify-center gap-2 transition-all transform hover:scale-[1.02] cursor-pointer"
            >
